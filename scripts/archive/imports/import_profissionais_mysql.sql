...
-- Schema atualizado: usa cpf, cns, cbo em PROD_Medico
-- Execução: mysql -u elizaelramos -p sigls_db < scripts/import_profissionais_mysql.sql
-- ============================================================================
...
SELECT CONCAT('Profissionais que já existem no sistema (por CPF): ', COUNT(*)) AS status
...
-- - id_origem será o CPF com prefixo 'medico_cpf_'
-- ============================================================================

-- PASSO 1: Atualizar profissionais existentes (por CPF)
...
-- id_origem = medico_cpf_<cpf> para garantir unicidade
...
SELECT CONCAT('Profissionais com CPF preenchido: ', COUNT(*)) AS resultado
FROM PROD_Medico WHERE cpf IS NOT NULL;

SELECT CONCAT('Total de vínculos no banco: ', COUNT(*)) AS resultado 
FROM Junction_Unidade_Medico;

SELECT CONCAT('Novos vínculos criados nesta importação: ', COUNT(*)) AS resultado
FROM Junction_Unidade_Medico j
INNER JOIN PROD_Medico m ON m.id = j.id_medico
WHERE m.cpf IN (SELECT cpf FROM profissionais_import_tmp);

SELECT CONCAT('Profissionais importados SEM vínculo: ', COUNT(*)) AS resultado
FROM PROD_Medico m
WHERE m.cpf IN (SELECT cpf FROM profissionais_import_tmp)
